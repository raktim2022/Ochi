...
function Navbar() {
  ...
  return (
      <motion.div 
        ...
        className={`backdrop-blur-sm fixed z-[999] w-full px-[55px] py-[15px] font-["Neue Montreal"] flex justify-between items-center ${scrolled ? 'bg-zinc-900/50' : ''}`}
      >
          <motion.div 
          ...
          <div className='links flex gap-10'>
              {["Services ", "Our Work ", "About US ", "Insights","Contact Us"].map((items, index) => (
                  <motion.a 
                    key={index} 
                    whileHover={{ scale: 1.1, color: "#CDEA68" }} 
                    transition={{ duration: 0.3 }}
                    className={`text-lg cursor-pointer capitalize font-light ${index===4 && "ml-32"}`}
                  >
                    {items}
                  </motion.a>
              ))}
